...
const customMin = ref('')
const customMax = ref('')
const filterMenuPosition = ref({ top: 0, left: 16 })
...
const updateFilterMenuPosition = () => {
  ...
  const menuWidth = viewportWidth >= 768 ? 480 : viewportWidth - 32
  const left =
    viewportWidth >= 768
      ? Math.max(16, Math.min(rect.left, viewportWidth - menuWidth - 16))
      : 16
...
<template>
  <div class="relative shrink-0">
    ...
    <Teleport to="body">
      ...
      <div
        v-if="showFilterMenu"
        class="fixed z-50 w-[calc(100vw-32px)] md:w-[480px] max-w-[480px] bg-white dark:bg-zinc-900 rounded-2xl shadow-xl border border-zinc-200 dark:border-zinc-700 flex flex-col max-h-[75svh] overflow-hidden"
      ...
      >
        ...
        <div class="overflow-y-auto custom-scrollbar p-4 space-y-4 md:space-y-5">
          <section>
