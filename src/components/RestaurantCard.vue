...
<template>
  ...
  >
    <!-- Cover Image -->
    <div class="h-full w-24 md:w-48 lg:w-64 shrink-0 overflow-hidden relative">
      <img
        :src="
    ...
    <div class="pt-1.5 px-3 pb-1.5 md:p-6 flex flex-col justify-between grow min-w-0">
      <div class="flex flex-col space-y-0.5 md:space-y-0">
        <div class="flex justify-between items-start md:mb-2">
          <h3 class="text-base md:text-lg font-bold text-zinc-900 dark:text-zinc-100 line-clamp-1">
            {{ restaurant.name }}
          </h3>
        </div>
        ...
        <div class="flex flex-wrap gap-1 md:gap-2">
          <span
            ...
            :key="tag"
            class="px-1 py-0.5 md:px-2 md:py-1 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[11px] md:text-xs rounded md:rounded-md"
          >
            #{{ tag }}
          </span>
        </div>
